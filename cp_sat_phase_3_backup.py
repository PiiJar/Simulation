...
def _load_phase1_snapshot(output_dir: str, phase_subdir: str = "cp_sat"):
    cp_dir = os.path.join(output_dir, phase_subdir)
...
class CpSatPhase2Optimizer:
    def __init__(self, output_dir: str, phase_subdir: str = "cp_sat", include_batches: Optional[List[int]] = None):
        ...
        ) = _load_phase1_snapshot(output_dir, phase_subdir)
    ...
    def _load_max_windows(self) -> Dict[int, int]:
        ...
        cp_dir = os.path.join(self.output_dir, self.phase_subdir)
...
def _copy_results_to_cp_sat(output_dir: str, phase_subdir: str):
    """Kopioi vaiheen tulokset cp_sat/ -kansioon (idioottivarma ratkaisu).
    
    Näin riippumatta siitä, ajettiinko Phase 2 vai Phase 3, tulokset löytyvät
    aina cp_sat/ -kansiosta jota muut skriptit käyttävät.
    """
    import shutil
    
    if phase_subdir == "cp_sat":
        # Jo oikeassa paikassa, ei tarvitse kopioida
        return
    
    src_dir = os.path.join(output_dir, phase_subdir)
    dst_dir = os.path.join(output_dir, "cp_sat")
    
    if not os.path.exists(src_dir):
        print(f"⚠️  Lähdekansiota ei löydy: {src_dir}")
        return
    
    os.makedirs(dst_dir, exist_ok=True)
    
    # Kopioi kaikki tiedostot ja alikansiot
    copied_count = 0
    for item in os.listdir(src_dir):
        ...
        try:
            ...
            print(f"⚠️  Virhe kopioitaessa {item}: {e}")
    
    print(f"\n✓ Kopioitu {copied_count} kohdetta: {phase_subdir}/ → cp_sat/")
    print(f"  Kaikki skriptit löytävät nyt tulokset: {dst_dir}/")


def optimize_phase_2(output_dir: str, phase_subdir: str = "cp_sat"):
    ...
    base = CpSatPhase2Optimizer(output_dir, phase_subdir)
    ...
    result = True if all_ok else None
    if result:
        _copy_results_to_cp_sat(output_dir, phase_subdir)
    return result
