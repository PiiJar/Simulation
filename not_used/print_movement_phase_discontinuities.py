...
def print_phase_discontinuities(output_root="output"):
    ...
    for transporter_id in sorted(df['Transporter'].unique()):
        ...
        for idx, row in t_df.iterrows():
            ...
            phase = row['Phase']
            if prev_batch == batch:
                # Oletetaan vaiheiden pitäisi edetä 0->1->2->3->4->0...
                if not (phase == prev_phase + 1 or (prev_phase == 4 and phase == 0)):
                    print(f"[EPÄJATKUVUUS] Nostin {transporter_id} | Batch {batch} | Rivillä {idx}: Vaihehyppy {prev_phase} -> {phase}")
                    print(df.loc[[prev_idx, idx]])
            prev_phase = phase
            prev_batch = batch
            prev_idx = idx

if __name__ == "__main__":
    print_phase_discontinuities()
