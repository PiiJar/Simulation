...
def cp_sat_transporter_centric(output_dir):
    ...
    # 2. Järjestetään transporter-tehtävät: jokainen siirto on yksi "reitti" transporterille
    # Luodaan muuttujat jokaiselle transporter-tehtävälle
    transporter_vars = []
    ...
    # 3. Transporterin tehtävien järjestys: AddNoOverlap
    ...
    # 4. Jokaiselle erän vaiheelle duration min/max-välissä, start ja end määräytyvät muiden rajoitteiden perusteella
    proc_vars = {}
    for var in transporter_vars:
        ...
        # Vaiheen alku = transporter-tehtävän loppu
        phase_start = var["end"]
        # Vaiheen duration min/max
        duration = model.NewIntVar(min_proc, max_proc, f"proc_dur_{batch}_{stage}")
        # Vaiheen loppu
        phase_end = model.NewIntVar(0, MAX_TIME, f"proc_end_{batch}_{stage}")
        model.Add(phase_end == phase_start + duration)
        proc_vars[(batch, stage)] = {"start": phase_start, "end": phase_end, "duration": duration}
        # Sidotaan transporter-tehtävän alku edellisen vaiheen loppuun (jos ei ensimmäinen vaihe)
        if stage > 0:
            prev = next((v for v in transporter_vars if v["task"]["batch"] == batch and v["task"]["this_stage"] == task["prev_stage"] and v["task"]["to_station"] == task["from_station"]), None)
            if prev:
                model.Add(var["start"] >= proc_vars[(batch, task["prev_stage"])] ["end"])
